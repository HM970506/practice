...
SortedLinkedList* SortedLinkedList_new()
{
	...
	_this->_size = 0; _this->_head = NULL;
	return  _this; 
}
...
{
	if (SortedLinkedList_isFull(_this)) return  FALSE; 
	else {
		Node* nodeForAdd = Node_new(); 
		Node_setElement(nodeForAdd, anElement); 
		Node_setNext(nodeForAdd, NULL); 

		if (SortedLinkedList_isEmpty(_this))  _this->_head = nodeForAdd ;
		else 
		{ 
			...
			{
				Node_setNext (nodeForAdd, _this->_head) ; _this->_head = nodeForAdd ; 
			}
...
{
	if (SortedLinkedList_isEmpty(_this)) return  ELEMENT_DOES_NOT_EXIST;
