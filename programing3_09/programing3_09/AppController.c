...
void AppController_run(AppController* _this)
{
	...
	while (c != Esc)
	{
		...
		if (isAlpha(c)) AppController_push(_this, c);
		else if (isDigit(c))
		{
			int  digitValue = c-'0';
		...
		else if (c == '-') AppController_pop1(_this);
		else if (c == '#') AppController_showSize(_this);
		else if (c == '/') AppController_showAllFromBottom(_this);
		else if (c == '\\') AppController_showAllFromTop(_this);
		else if (c == '^') AppController_showTopElement(_this);
...
{
	if (Stack_isFull(_this->_stack)) AppView_out_stackIsFullAgainstPush(aChar);
	else {
...
{
	if (Stack_isEmpty(_this->_stack)) AppView_out_stackIsEmptyAgainstPop1();
	else {
		char  poppedChar = Stack_pop(_this->_stack);
...
{
	AppView_out_topElement(_this->_stack->_elements[_this->_stack->_top]);
}
...
void AppController_endInput(AppController* _this)
{
	...
	for (int i = -1; (i <= _this->_stack->_top); i++)
	{
		if (Stack_isEmpty(_this->_stack)) return;
