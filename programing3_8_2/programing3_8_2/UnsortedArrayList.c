...
{
	return (_this->_size == 0); //0일때 true로 리턴
}
...
int UnsortedArrayList_maxPositionRecursively(UnsortedArrayList* _this, int left, int right) //BS
{
	...
	else
	{
		...
		else return maxPositionOfRightPart;
...
int  UnsortedArrayList_minPositionRecursively(UnsortedArrayList* _this, int left, int right)
{
	...
	else
	{
		...
		if (_this->_elements[minPositionOfLeftPart] <= _this->_elements[minPositionOfRightPart]) return minPositionOfLeftPart;
		else return minPositionOfRightPart;
